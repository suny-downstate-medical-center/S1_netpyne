...
synNumber2 = {}
for pre in range(55):
    for post in range(1035):
        synNumber2[pre,post] = 0
...
for line in data:
    ...
    # synNumber2[line[3],line[0]] = synNumber2[line[3],line[0]] + 1
    synT[line[3]][line[14]][int(line[0])] = {}
    synT2[line[3]][line[14]][int(line[0])] = {}

for line in data:
    synT[line[3]][line[14]][int(line[0])][int(line[2])] = 0
    synT2[line[3]][line[14]][int(line[0])][int(line[2])] = 0

for line in data:
    synT[line[3]][line[14]][int(line[0])][int(line[2])] = synT[line[3]][line[14]][int(line[0])][int(line[2])] + 1

for line in data:
    synT2[line[3]][line[14]][int(line[0])][int(line[2])] = synT2[line[3]][line[14]][int(line[0])][int(line[2])] + 1
    
    if synT2[line[3]][line[14]][int(line[0])][int(line[2])] == synT[line[3]][line[14]][int(line[0])][int(line[2])]:
        # print(int(line[3]),int(line[14]),int(synT[line[3]][line[14]][int(line[0])][int(line[2])]))
        synNumber[int(line[3]),int(line[14])] = synNumber[int(line[3]),int(line[14])] + int(synT[line[3]][line[14]][int(line[0])][int(line[2])])
        synType[int(line[3]),int(line[14])] = synType[int(line[3]),int(line[14])] + 1

        synNumber2[int(line[3]),int(line[14])] = synNumber2[int(line[3]),int(line[14])] + int(synT[line[3]][line[14]][int(line[0])][int(line[2])])*int(synT[line[3]][line[14]][int(line[0])][int(line[2])])


for pre in range(55):
    for post in range(55):
        if synType[pre,post] > 0:
            mean = synNumber[pre,post]/synType[pre,post]
            mean2 = synNumber2[pre,post]/synType[pre,post]
            root = np.sqrt(mean2 - mean*mean)
            print('%d %d %.1f %.1f %d %s:%s' % (pre,post,mean,root,synType[pre,post],mtype_map2[pre],mtype_map2[post]))

# print(synT)
# for pre in range(55):
#     for post in range(55):
#         if synNumber[pre,post] > 0:    
#             for post2 in range(1035):            
#                 if synNumber2[pre,post2] > 0:
#                     print(pre,post,post2,synT[pre][post][post2])  
