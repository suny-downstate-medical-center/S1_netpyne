...
** rerun and try again -->> working on reduced model with 60 cores
...
try with just using cfg.scale ... or add a diff option for scaling the thalamic cells ... 

ok, adding cfg.scaleThal ... will try with 1/20 first ... 
...
simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname)
drawraster(dspkT,dspkID)
savefig('gif/22mar21_rast_a1.png')

ok, thal not as dominant now ... 

could reduce thal some more and then put back more of S1 cells ...

mpiexec -n 60 --use-hwthread-cpus nrniv -python -mpi init.py

python -i simdat.py
simname = '22mar22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
savefig('gif/22mar21_rast_a2.png')

and here's total dipole signal: gif/22mar22_LDA_dipole.png

try running for 3s to see if activity persists ... 

mpiexec -n 60 --use-hwthread-cpus nrniv -python -mpi init.py

hmm, crashes at gatherdata when running for 3 s ... :

3.0s
  Done; run time = 809.43 s; real-time ratio: 0.00.

Gathering data...
[cycle:110342] *** Process received signal ***
[cycle:110342] Signal: Segmentation fault (11)
[cycle:110342] Signal code:  (216521728)
[cycle:110342] Failing at address: 0x2420
...
ok, that did not crash ... but need to run longer than 500 ms ... 

python -i simdat.py
simname = '22mar22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
savefig('gif/22mar21_rast_a3.png')

pip install -U netpyne

upgraded from netpyne-1.0.1 to netpyne-1.0.2 ... will see if works w/o crash ... 

mpiexec -n 60 --use-hwthread-cpus nrniv -python -mpi init.py

when saving data in nodes does not crash but does not merge the files together properly ... 
...
could manually merge ... ? 

w/o population dipoles ... and standard gather, somewhat smaller network, seems to work

mpiexec -n 60 --use-hwthread-cpus nrniv -python -mpi init.py

python -i simdat.py
simname = '22mar22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
savefig('gif/22mar21_rast_a4.png')

plot(sdat['t'][0:-1],sdat['dipoleSum'])

savefig('gif/22mar21_sumdpl_a5.png')

ok ... 

for k in sdat['dipoleCells'].keys(): plot(sdat['t'][0:-1],sdat['dipoleCells'][k][:,0])
...
ok, that ran to completion ...

python -i simdat.py
simname = '22mar22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
savefig('gif/22mar21_rast_a6.png')

ok, activity persists...due to the spontaneous inputs

plot(sdat['t'],sdat['dipoleSum'])
savefig('gif/22mar21_sumdpl_a7.png')

are the x,y,z coordinates available?

simConfig['net'].keys()
simConfig['net']['cells'][0]
yes, has the xnorm, ynorm, znorm ... and also the x, y, z coordinates, presumably in microns

** dipole->matlab code from Sal in A1 model: https://github.com/NathanKlineInstitute/A1/blob/salva_layers/analysis/disc_grant.py#L368

can use/adjust the save_dipoles_matlab function
...
./myrun 60
 ...
 Done; run time = 2893.52 s; real-time ratio: 0.01.
Gathering data...
terminate called after throwing an instance of 'std::bad_alloc'
  what():  std::bad_alloc
[cycle:205025] *** Process received signal ***
[cycle:205025] Signal: Aborted (6)
[cycle:205025] Signal code:  (-6)

with gatherDataFromFiles probably won't crash but looking at the code in /opt/anaconda3/lib/python3.7/site-packages/netpyne/sim/gather.py
that function does not have gatherDipole option similar to the gatherDipole option in standard gatherData . that could explain the other crash
when using gatherDataFromFiles ... so would have to fix that function to allow gather dipole to work properly . . . ?

first make sure still getting error with the distributed save ... 

./myrun 60

yes, sim runs to completion but throws an exception during gather data from files 

* 23mar22
** check data to see if can merge

python -i simdat.py

d = pickle.load(open('../data/22mar22_LDA/22mar22_LDA_node_data/22mar22_LDA_node_0.pkl','rb'))
dict_keys(['netpyne_version', 'netpyne_changeset', 'simConfig', 'simData', 'cells', 'pops', 'net'])

d['simData'].keys() # dict_keys(['spkt', 'spkid', 'V_soma', 't', 'dipoleSum', 'dipoleCells'])
d['simData']['dipoleCells'].keys() # dict_keys([0, 60, 120, 180, 240, 300, 360, 420, 480])

ok, so all the individual data is there ... just spread across multiple files 

plot(d['simData']['t'][0:-1],d['simData']['dipoleCells'][0][:,0])

savefig('gif/23mar22_a0.png')
savefig('gif/23mar22_a0b.png')

hmm, part of that signal looks like saturating ... ??

plot(d['simData']['t'][0:-1],d['simData']['dipoleCells'][60][:,0],'r')

savefig('gif/23mar22_a0c.png')

hmm, that strange saturation/high-freq activity occurs for multiple cells, so could be a bug ... 

plot(d['simData']['t'][0:-1],d['simData']['dipoleCells'][120][:,0],'g')

savefig('gif/23mar22_a0d.png')

plot(d['simData']['spkt'],d['simData']['spkid'],'ko')

plot(d['simData']['t'][0:-1],d['simData']['dipoleSum'])
savefig('gif/23mar22_a0e.png')

especially since the dipole integrated across cells does not have that artifact ...

d = pickle.load(open('../data/22mar22_LDA/22mar22_LDA_node_data/22mar22_LDA_node_1.pkl','rb'))
d['simData']['dipoleCells'].keys() # dict_keys([1, 61, 121, 181, 241, 301, 361, 421, 481])
plot(d['simData']['dipoleCells'][61][:,0],'r')
savefig('gif/23mar22_a0f.png')
no artifact from this node ... 

d = pickle.load(open('../data/22mar22_LDA/22mar22_LDA_node_data/22mar22_LDA_node_0.pkl','rb'))
d['net'].keys() # {}
empty ... so will need net,pop,cell,gid info from full pkl file ... 

** avoiding dipole,LFP artifact -->> use same value for recordStep and simulation dt

EG said that to avoid artifact in LFP (which looked similar to these dipole artifacts), they
set recordstep and dt to same values

try with recordStep and dt at 0.1 ... only run for 6 s ... 

mpiexec -n 60 --use-hwthread-cpus nrniv -python -mpi init.py

ran a lot faster with the longer dt ... 

python -i simdat.py

d = pickle.load(open('../data/23mar22_LDA/23mar22_LDA_node_data/23mar22_LDA_node_0.pkl','rb'))
d['simData']['dipoleCells'].keys() # dict_keys([0, 60, 120, 180, 240, 300, 360, 420, 480])

plot(d['simData']['t'][0:-1],d['simData']['dipoleCells'][0][:,0])
savefig('gif/23mar22_b0.png')

artifact is gone ... 

should check if gatherData works w/o crash with this larger dt ... 

mpiexec -n 60 --use-hwthread-cpus nrniv -python -mpi init.py

great...that did not crash ... 

python -i simdat.py

d = pickle.load(open('../data/23mar22_LDA/23mar22_LDA_data.pkl','rb'))

plot(d['simData']['t'][0:-1],d['simData']['dipoleCells'][0][:,0])
savefig('gif/23mar22_b1.png')

ok...will try a 10 s sim ... 

mpiexec -n 60 --use-hwthread-cpus nrniv -python -mpi init.py

  Spikes: 25370 (4.95 Hz)
  Simulated time: 10.0 s; 60 workers
  Run time: 1510.38 s

great, that saved properly ... 

python -i simdat.py
simname = '23mar22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
savefig('gif/23mar22_rast_b2.png')

plot(sdat['t'],sdat['dipoleSum'])
savefig('gif/23mar22_sumdpl_b3.png')

plot(sdat['t'],sdat['dipoleCells'][0][:,0])
savefig('gif/23mar22_dpl_b4.png')

looks good, no artifacts ... as for firing rates, some populations look low ... 

* 24mar22
** install dev branch of netpyne (has some fixes to gatherData, gatherDataFromFiles)
...
** try dev branch of netpyne -- to check if gatherDataFromFiles works with the dipoles
...
Searching for .pkl node files in ../data/24mar22_LDA/24mar22_LDA_node_data ...

Gathering data from files for simulation: 24mar22_LDA ...
  Merging data file: 24mar22_LDA_node_0.pkl
>>> 
Traceback (most recent call last):
  File "init.py", line 34, in <module>
    sim.gatherDataFromFiles()
  File "/home/samn/netpyne/netpyne/sim/gather.py", line 473, in gatherDataFromFiles
    allSimData[key].update(value)
ValueError: dictionary update sequence element #0 has length 3; 2 is required
>>> import netpyne
>>> netpyne.__version__
...
** continue - examine previous output

make sure can get the x,y,z of each cortical cell ... as well as their dipoles ... 
...
simConfig['net']['cells'][0]['tags']['x']
GetCellCoords(simConfig,0) # [18.89076276824184, 132.88113983393526, 3.298729009722557]

save_dipoles_matlab('test.mat', simConfig, sdat, dnumc, dstartidx, dendidx)
...
cfg.addTargetedNetStim

start at 1000 ... apply 3Hz stims (3 times), then turn off and run until t=3000
try with noise=0.25

weights have to be pretty high ... original placeholder value of 20 way too low to get any kind
of response ... ~10e3 - 25e3 gets a strong response ... 10e3 is sub-threshold but strong
and 25e3 is superthreshold

mpiexec -n 60 --use-hwthread-cpus nrniv -python -mpi init.py

python -i simdat.py
simname = '28mar22_StimVPL_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,3e3],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=4)
plot([0,3e3],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=4)
figure(); plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx['VPL_sTC'])])

ok, can see a response in VPL_sTC, but not much spreading elsewhere ...

[[/home/samn/S1_netpyne/sim/gif/28mar22_rast_a0.png]]
[[/home/samn/S1_netpyne/sim/gif/28mar22_cell_a1.png]]
...
* 31mar22
** check output of thal stim simulation

python -i simdat.py
simname = '28mar22_StimVPL_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)

plot([0,10e3],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=4)
plot([0,10e3],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=4)
savefig('gif/31mar22_rast_a0.png')

ok, stim seems to be working properly ... might be some spread to ctx ... can see patterned activity at the
stimulation rate ... though since populations are small, somewhat difficult to see in the raster

figure(); plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx['VPL_sTC'])])
savefig('gif/31mar22_VPL_Vm_a1.png')

also check a layer 4 pyramidal cell
dnumc['L4_PC_cAD'] # 19

ty='L4_PC_cAD'; plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx[ty])],'r')
savefig('gif/31mar22_VPL_L4_PC_Vm_a2.png')

getrate(dspkT,dspkID,ty,dnumc) # 0.9210526315789473
getrate(dspkT,dspkID,'VPL_sTC',dnumc) # 4.109090909090909

for ty in ['VPL_sTC', 'L4_PC_cAD']: print(ty,getrate(dspkT,dspkID,ty,dnumc))

VPL_sTC   4.109090909090909
L4_PC_cAD 0.9210526315789473

ok, and compare firing rates to simulation w/o stimulation ... 
...
VPL_sTC 1.1045454545454545
L4_PC_cAD 0.9210526315789473

hmm, VPL firing rate is lower w/o VPL stimulation, but L4 firing rates same ... so that means activity
is not propagating from thal -> S1 ... 

savefig('gif/31mar22_VPL_L4_PC_Vm_a3.png')

that might be due to lower number of thal cells in reduced model ... so could scale up the weights from thal -> S1 ... 

** new param cfg.Th_S1_Gain 

this scales synaptic weights from Th -> S1 (used in netParams.py)

** test increase cfg.Th_S1_Gain
*** with cfg.Th_S1_Gain == 10

./myrun 60 

python -i simdat.py
simname = '31mar22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/31mar22_rast_b0.png')

for ty in ['VPL_sTC', 'L4_PC_cAD']: print(ty,getrate(dspkT,dspkID,ty,dnumc))
VPL_sTC 3.742424242424242
L4_PC_cAD 2.017543859649123

ok, now there's strong rhythmic thalamocortical propagation ... but stim of VPL not making much of a change to
overall pattern ... 

for ty,clr in zip(['VPL_sTC','L4_PC_cAD'],['b','r']): plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx[ty])],clr)
savefig('gif/31mar22_Vm_b1.png')

*** with cfg.Th_S1_Gain == 5
...
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/31mar22_rast_c0.png')

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD']: print(ty,getrate(dspkT,dspkID,ty,dnumc))
VPM_sTC 3.3333333333333335
VPL_sTC 3.606060606060606
L4_PC_cAD 1.4385964912280702

for ty,clr in zip(['VPM_sTC', 'VPL_sTC','L4_PC_cAD'],['g','b','r']): plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx[ty])],clr)
savefig('gif/31mar22_Vm_c1.png')

ok, looks like some synchronization of POm driving most of the propagation from TC -> S1; so should probably wait
~5 s until start stimulation ... could run 15 s sim, with 5 s initialization, and 10 s of stimulation, then 5 s w/o
further stimulation ... 

** apply vb patch for distributed save, relevant for dipoles

put this into ~/netpyne/dipoleSum.patch:

diff --git a/netpyne/sim/gather.py b/netpyne/sim/gather.py
index f093ecbd..598cafa7 100644
...
cd ~/netpyne
git apply dipoleSum.patch
 
** test VPL stim in longer simulation

15 s simulation, 5 s of stimulation in middle 5 s ... hmm, that crashes in gatherdata ... try with
vb patch above for distributed save ... ok , applied patch, can try again 
(... to avoid initial synchronization transient could adjust
netstims to start at random times during first few s ...)

./myrun 60

ok, with the fix, distributed saving works ... 

python -i simdat.py
simname = '31mar22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/31mar22_rast_d0.png')

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD']: print(ty,getrate(dspkT,dspkID,ty,dnumc))

for ty,clr in zip(['VPM_sTC', 'VPL_sTC','L4_PC_cAD'],['g','b','r']): plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx[ty])],clr)
savefig('gif/31mar22_Vm_d1.png')

simname = '31mar22_LDA'
d = pickle.load(open('31mar22_LDA_data.pkl','rb'))
for p in d['net']['pops'].keys(): print(p,d['net']['pops'][p]['numCells'],len(d['net']['pops'][p]['cellGids']))

hmm, some discrepancy in the len of cellGids and the number of cells . . . 

may try starting netstims at different times ... 

first will try run for 10 s ... w/o distributed save. and only have stim for last 5 s.
...
looks rhythmic already ... 

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD']: print(ty,getrate(dspkT,dspkID,ty,dnumc))

VPM_sTC 2.007142857142857
VPL_sTC 2.227272727272727
L4_PC_cAD 1.1736842105263159

for ty,clr in zip(['VPM_sTC', 'VPL_sTC','L4_PC_cAD'],['g','b','r']): plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx[ty])],clr)
savefig('gif/31mar22_Vm_d1.png')

can try lower strength off / off stim too ...

* 1apr22
** setup only record dipoles from S1

this might work, in netParams.py:
if cfg.recordDipole: cfg.saveDipoleCells = cfg.S1pops
...
Traceback (most recent call last):
  ...
  File "/home/samn/netpyne/netpyne/sim/setup.py", line 518, in setupRecording
    setupRecordDipole()
  File "/home/samn/netpyne/netpyne/sim/setup.py", line 377, in setupRecordDipole
    cellsRecordDipole = utils.getCellsList(sim.cfg.saveDipoleCells)
  File "/home/samn/netpyne/netpyne/sim/utils.py", line 98, in getCellsList
    cellGids.extend(list(sim.net.pops[condition].cellGids))
  File "/home/samn/netpyne/netpyne/specs/dicts.py", line 197, in __getitem__
    return super(ODict, self).__getitem__(k)
KeyError: 'L1_DLAC'

L1_DAC causes a key error ... are there no L1_DAC cells??
happens for all of the populations ... ah, error since sim.net.pops not setup yet
during init.py ?? no, have createPops, createCells before ... maybe error since
not all pops exist on each node ?

no, was not using full population name ... need to do like this:
if cfg.recordDipole: cfg.saveDipoleCells = cfg.S1cells

cfg.S1cells has full cell type name ...

so, that works ... 

ok, try that for 15 s ... 

** try 15 s sim again, with stimulation for 5 s

./myrun 60

ok, that ran w/o crashing...

python -i simdat.py
simname = '1apr22_LD_StimHigh'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/1apr22_rast_a0.png')

seems like rhythmic activity already present ... so, a bit difficult to distinguish from stimulus
maybe worth doubling stimulus rate ... or the weights from VPL -> S1 ... 

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD']: print(ty,getrate(dspkT,dspkID,ty,dnumc))

VPM_sTC 1.9404761904761905
VPL_sTC 2.1484848484848484
L4_PC_cAD 1.119298245614035

for ty,clr in zip(['VPM_sTC', 'VPL_sTC','L4_PC_cAD'],['g','b','r']): plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx[ty])],clr)

savefig('gif/1apr22_Vm_a1.png')

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD']:
  for tlim in [(1e3,5e3),(5e3,10e3),(10e3,15e3)]:
    print('tlim:',np.array(tlim)/1e3,ty,getrate(dspkT,dspkID,ty,dnumc,tlim=tlim))

tlim: [1. 5.]   VPM_sTC 3.0982142857142856
tlim: [ 5. 10.] VPM_sTC 1.0928571428571427
tlim: [10. 15.] VPM_sTC 0.6214285714285714
tlim: [1. 5.]   VPL_sTC 0.7045454545454546
tlim: [ 5. 10.] VPL_sTC 4.709090909090909
tlim: [10. 15.] VPL_sTC 0.5545454545454546
tlim: [1. 5.]   L4_PC_cAD 1.0394736842105263
tlim: [ 5. 10.] L4_PC_cAD 1.0526315789473684
tlim: [10. 15.] L4_PC_cAD 1.0105263157894737

barely any change in firing rates of non-stimulated populations ... may need to adjust Thal -> S1 weights further
or use different stim rate ... will first try 6 Hz stim instead ... 
...
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/1apr22_rast_a2.png')

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD']: print(ty,getrate(dspkT,dspkID,ty,dnumc))
VPM_sTC 2.85
VPL_sTC 2.790909090909091
L4_PC_cAD 1.1298245614035087

for ty,clr in zip(['VPM_sTC', 'VPL_sTC','L4_PC_cAD'],['g','b','r']): plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx[ty])],clr)

savefig('gif/1apr22_Vm_a3.png')

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD', 'L23_PC_cAD']:
  for tlim in [(1e3,5e3),(5e3,10e3),(10e3,15e3)]:
    print('tlim:',np.array(tlim)/1e3,ty,getrate(dspkT,dspkID,ty,dnumc,tlim=tlim))

tlim: [1. 5.]   VPM_sTC 5.946428571428571
tlim: [ 5. 10.] VPM_sTC 0.7428571428571429
tlim: [10. 15.] VPM_sTC 0.9714285714285714
tlim: [1. 5.]   VPL_sTC 0.7045454545454546
tlim: [ 5. 10.] VPL_sTC 7.1
tlim: [10. 15.] VPL_sTC 0.09090909090909091
tlim: [1. 5.]   L4_PC_cAD 1.0394736842105263
tlim: [ 5. 10.] L4_PC_cAD 1.0736842105263158
tlim: [10. 15.] L4_PC_cAD 1.0210526315789474
tlim: [1. 5.]   L23_PC_cAD 0.4146341463414634
tlim: [ 5. 10.] L23_PC_cAD 0.526829268292683
tlim: [10. 15.] L23_PC_cAD 0.3951219512195122

hmm, VPL almost entirely off after the stim ... maybe due to ADP

should S1 -> Thal weights increase too? 

what happens w/o scaling thal down ... ? 

** try less scaling down thal

will probably crash in gatherdata ... 

with 2x thal size of last time (scale down by 1/30 instead of 1/60)

./myrun 60

and a 3 s sim ... 6 Hz stim rate ... 
and 2.5 X gain (instead of 5x) from Thal -> S1

3 s runs to completion ... 

* 4apr22
** suggestions from FB

FB thinks in full scale model most of the S1 dynamics governed by
spontaneous inputs and Thal -> S1 connectivity; tested by removing
S1 <-> S1 connections and checking if dynamics differed (they did not)

** next sims - larger thal

keeping thal a little larger and original Thal -> S1 weights to test 
first try w/o targeted stim, to see if synchrony in thal dissipates

./myrun 60

python -i simdat.py
simname = '4apr22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/4apr22_rast_a0.png')

still a lot of thalamic synchrony, and still transfers (at least partially) to S1

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD']: print(ty,getrate(dspkT,dspkID,ty,dnumc))

VPM_sTC 3.480722891566265
VPL_sTC 3.4107692307692306
L4_PC_cAD 0.9263157894736842

for ty,clr in zip(['VPM_sTC', 'VPL_sTC','L4_PC_cAD'],['g','b','r']): plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx[ty])],clr)

savefig('gif/4apr22_Vm_a1.png')

try 15 s sim ... see if runs to completion ... 
...
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/4apr22_rast_a2.png')

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD', 'L23_PC_cAD']:
  for tlim in [(1e3,5e3),(5e3,10e3),(10e3,15e3)]:
    print('tlim:',np.array(tlim)/1e3,ty,getrate(dspkT,dspkID,ty,dnumc,tlim=tlim))

tlim: [1. 5.] VPM_sTC 2.8463855421686746
tlim: [ 5. 10.] VPM_sTC 3.330120481927711
tlim: [10. 15.] VPM_sTC 3.9036144578313254
tlim: [1. 5.] VPL_sTC 2.0692307692307694
tlim: [ 5. 10.] VPL_sTC 3.6953846153846155
tlim: [10. 15.] VPL_sTC 3.766153846153846
tlim: [1. 5.] L4_PC_cAD 0.8947368421052632
tlim: [ 5. 10.] L4_PC_cAD 0.8631578947368421
tlim: [10. 15.] L4_PC_cAD 0.8947368421052632
tlim: [1. 5.] L23_PC_cAD 0.2804878048780488
tlim: [ 5. 10.] L23_PC_cAD 0.2780487804878049
tlim: [10. 15.] L23_PC_cAD 0.28780487804878047

lot of variability in firing rates ... notice that VPL goes in and out of firing activity ... 

save_dipoles_matlab('/home/samn/S1_netpyne/data/4apr22_LDA/dipoles.mat', simConfig, sdat, dnumc, dstartidx, dendidx)

plot(sdat['t'],sdat['dipoleSum'])
xlim((0,15e3))
savefig('gif/4apr22_dipoleSum_a3.png')

* 5apr22
** run same sim with strong 3 Hz stim to VPL (5apr22_StrongStimVPL3Hz_LDA)

will use 0.1 noise, start stim at 1e3, end stim at 14e3 ... total sim duration of 15e3
stim strength of 25e3 

./myrun 60

ran to completion...

  Simulated time: 15.0 s; 60 workers
  Run time: 2634.08 s

python -i simdat.py
simname = '5apr22_StrongStimVPL3Hz_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/5apr22_rast_b0.png')

hmm, cant see any rhythm with the noise == 0.1 ... but the firing patterns are very different in thalamus ... 

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD', 'L23_PC_cAD']:
  for tlim in [(1e3,5e3),(5e3,10e3),(10e3,15e3)]:
    print('tlim:',np.array(tlim)/1e3,ty,getrate(dspkT,dspkID,ty,dnumc,tlim=tlim))

lim: [1. 5.] VPM_sTC 3.3283132530120483
tlim: [ 5. 10.] VPM_sTC 2.9493975903614458
tlim: [10. 15.] VPM_sTC 3.6409638554216865
tlim: [1. 5.] VPL_sTC 4.676923076923077
tlim: [ 5. 10.] VPL_sTC 3.7046153846153844
tlim: [10. 15.] VPL_sTC 4.547692307692308
tlim: [1. 5.] L4_PC_cAD 0.8552631578947368
tlim: [ 5. 10.] L4_PC_cAD 0.8947368421052632
tlim: [10. 15.] L4_PC_cAD 0.8736842105263158
tlim: [1. 5.] L23_PC_cAD 0.2804878048780488
tlim: [ 5. 10.] L23_PC_cAD 0.28292682926829266
tlim: [10. 15.] L23_PC_cAD 0.28292682926829266

# save_dipoles_matlab('/home/samn/S1_netpyne/data/4apr22_LDA/dipoles.mat', simConfig, sdat, dnumc, dstartidx, dendidx)

plot(sdat['t'],sdat['dipoleSum']); 
xlim((0,15e3))
savefig('gif/5apr22_dipoleSum_b1.png')

simname = '4apr22_LDA'; simConfig, sdatB, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); 
cla(); plot(sdatB['t'],sdatB['dipoleSum'][:,0],'b'); plot(sdat['t'],sdat['dipoleSum'][:,0],'r'); 
xlim((5e3,6e3))

savefig('gif/5apr22_dipoleSum_compare_b2.png')

hmm, not identical but very similar ... 

try again w/o any noise in VPL stim ... 
...
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/5apr22_rast_b3.png')

ok, now can see clear response in VPL ... 

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD', 'L23_PC_cAD']:
  for tlim in [(1e3,5e3),(5e3,10e3),(10e3,15e3)]:
    print('tlim:',np.array(tlim)/1e3,ty,getrate(dspkT,dspkID,ty,dnumc,tlim=tlim))

tlim: [1. 5.] VPM_sTC 3.3283132530120483
tlim: [ 5. 10.] VPM_sTC 2.9493975903614458
tlim: [10. 15.] VPM_sTC 3.6409638554216865
tlim: [1. 5.] VPL_sTC 6.461538461538462
tlim: [ 5. 10.] VPL_sTC 5.2153846153846155
tlim: [10. 15.] VPL_sTC 7.578461538461538
tlim: [1. 5.] L4_PC_cAD 0.8552631578947368
tlim: [ 5. 10.] L4_PC_cAD 0.8842105263157894
tlim: [10. 15.] L4_PC_cAD 0.8842105263157894
tlim: [1. 5.] L23_PC_cAD 0.2804878048780488
tlim: [ 5. 10.] L23_PC_cAD 0.28292682926829266
tlim: [10. 15.] L23_PC_cAD 0.28292682926829266
...
savefig('gif/5apr22_dipoleSum_compare_b4.png')

still not that different ... may need to adjust gains from TH -> S1 ... 

** next sims/adjustments

may as well reduce thal again, since larger scale thal shows similar dynamics

also noticed an indent typo in netParams.py (from S1 -> TC); fixed that up and
added cfg.S1_Th_Gain option

STP (short term plasticity) seems to be used in this model; mod files are DetAMPANMDA.mod and DetGABAAB.mod
may want to turn it off

SD mentions that the STP contributes to slow ~1 Hz rhythm , so can keep it here for consistency with
the full model

** setup next sim

gain of 1.5x, 1.5x between Thal and S1
spontaneous
...
python -i simdat.py
simname = '5apr22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/5apr22_rast_c0.png')

VPL very quiet...and possibly too much synchrony between Thal, S1 ... 

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD', 'L23_PC_cAD']:
  for tlim in [(1e3,5e3),(5e3,10e3),(10e3,15e3)]:
    print('tlim:',np.array(tlim)/1e3,ty,getrate(dspkT,dspkID,ty,dnumc,tlim=tlim))

tlim: [1. 5.] VPM_sTC 0.9910714285714286
tlim: [ 5. 10.] VPM_sTC 0.21428571428571427
tlim: [10. 15.] VPM_sTC 0.5642857142857143
tlim: [1. 5.] VPL_sTC 1.0568181818181819
tlim: [ 5. 10.] VPL_sTC 0.7909090909090909
tlim: [10. 15.] VPL_sTC 0.9363636363636364
tlim: [1. 5.] L4_PC_cAD 0.8947368421052632
tlim: [ 5. 10.] L4_PC_cAD 0.8947368421052632
tlim: [10. 15.] L4_PC_cAD 0.9052631578947369
tlim: [1. 5.] L23_PC_cAD 0.29878048780487804
tlim: [ 5. 10.] L23_PC_cAD 0.28292682926829266
tlim: [10. 15.] L23_PC_cAD 0.28780487804878047

cla(); plot(sdat['t'],sdat['dipoleSum'][:,0],'k'); 
xlim((5e3,6e3))

savefig('gif/5apr22_dpl_c1.png')

reduce the S1, Thal gains to 1.25 ... try again ... 

try at 2.5 since dont see much propagation ... 
...
python -i simdat.py
simname = '5apr22_Stim_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/5apr22_rast_c2.png')

for ty in ['VPM_sTC', 'VPL_sTC', 'L4_PC_cAD', 'L23_PC_cAD']:
  for tlim in [(0,1e3),(1e3,14e3),(14e3,15e3)]:
    print('tlim:',np.array(tlim)/1e3,ty,getrate(dspkT,dspkID,ty,dnumc,tlim=tlim))

tlim: [0. 1.] VPM_sTC 10.392857142857142
tlim: [ 1. 14.] VPM_sTC 5.782967032967033
tlim: [14. 15.] VPM_sTC 2.75
tlim: [0. 1.] VPL_sTC 2.8636363636363638
tlim: [ 1. 14.] VPL_sTC 7.171328671328672
tlim: [14. 15.] VPL_sTC 0.5
tlim: [0. 1.] L4_PC_cAD 1.9473684210526316
tlim: [ 1. 14.] L4_PC_cAD 0.9352226720647774
tlim: [14. 15.] L4_PC_cAD 0.9473684210526315
tlim: [0. 1.] L23_PC_cAD 1.2195121951219512
tlim: [ 1. 14.] L23_PC_cAD 0.3151969981238274
tlim: [14. 15.] L23_PC_cAD 0.3170731707317073

cla(); plot(sdat['t'],sdat['dipoleSum'][:,0],'k'); 
xlim((5e3,6e3))

savefig('gif/5apr22_dpl_c3.png')

need to compare to sim w/o stimulation ... to see if there's any impact

* 6apr22
** run same sim w/o the stimulation (6apr22_LDA)
...
python -i simdat.py
simname = '6apr22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/6apr22_rast_a0.png')

cla(); plot(sdat['t'][:-1],sdat['dipoleSum'][:,0],'b'); 
xlim((1e3,2e3))

savefig('gif/6apr22_dpl_a1.png')

simname = '5apr22_Stim_LDA'; simConfig, sdatB, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); 
plot(sdatB['t'],sdatB['dipoleSum'][:,0],'r'); 

savefig('gif/6apr22_dpl_a2.png')

again, not seeing much difference in the dipole sum ... 

may need to adjust VPL -> S1 pathway ... 

** check/adjust VPL -> S1 pathway (6apr22_Stim_LDB)

POm vs VPL, VPM use different connectivity rules ... VPL, VPM use some form of topographical
connectivity?, whereas POm (higher order inputs) use different?
...
since dealing with a much smaller area in the reduced model may want to use convergence that is not
spatially dependent ... 

try with this adjustment:
...
try this sim, run for 3 s with 6 Hz targeted stim to VPL

python -i simdat.py
simname = '6apr22_Stim_LDB'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/6apr22_rast_b0.png')

yeah, that looks a lot different. now VPL has a clear impact on S1

cla(); plot(sdat['t'][:-1],sdat['dipoleSum'][:,0],'b'); 

savefig('gif/6apr22_dpl_b1.png')

now try with slightly lower gain on weights between S1, Thal ... (1.5 instead of 2.5)

python -i simdat.py
simname = '6apr22_Stim_LDB'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/6apr22_rast_b2.png')

even at this lower gain, effects still noticeable

cla(); plot(sdat['t'][:-1],sdat['dipoleSum'][:,0],'b'); 

savefig('gif/6apr22_dpl_b3.png')

could reduce those gains (cfg.Th_S1_Gain, cfg.S1_Th_Gain) to original values (1.0, 1.0) and see how looks ... 

python -i simdat.py
simname = '6apr22_Stim_LDB'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/6apr22_rast_b4.png')

cla(); plot(sdat['t'][:-1],sdat['dipoleSum'][:,0],'b'); 

savefig('gif/6apr22_dpl_b5.png')

still have a pretty strong/visible impact ... 

could reduce strength of stimulus to VPL, and or use some noise ... 
...
savefig('gif/6apr22_rast_b6.png')
cla(); plot(sdat['t'][:],sdat['dipoleSum'][:,0],'b'); 
savefig('gif/6apr22_dpl_b7.png')

looks better with the noise = 0.01, as long as the NetStim inputs to VPL come in ~same time ... and doesn't
drift over a long simulation ... 

try 15 s sim now ... with same stim params ...
...
savefig('gif/6apr22_rast_b6.png')
cla(); plot(sdat['t'][:],sdat['dipoleSum'][:,0],'b'); 
savefig('gif/6apr22_dpl_b8.png')

looks pretty reasonable overall ... have that initial spike from first stim but the others are not as large, though
still visible

so now run comparison sim w/o the stimulation ... 

simname = 6apr22_LDB

python -i simdat.py

simname = '6apr22_LDB'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)

savefig('gif/6apr22_rast_b9.png')
cla(); plot(sdat['t'][:],sdat['dipoleSum'][:,0],'b'); 
savefig('gif/6apr22_dpl_b10.png')

simname = '6apr22_Stim_LDB'; simConfig, sdatB, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); 
plot(sdatB['t'],sdatB['dipoleSum'][:,0],'r'); 

savefig('gif/6apr22_dpl_b11.png')

can see some differences ... due to noisiness, not as pronounced ... could set noise=0 to compare

or check spectrograms ... 

len(sdatB['dipoleSum'][:,0])

from morlet import *

a,b = sdat['dipoleSum'][10000:,0],sdatB['dipoleSum'][10000:,0]
a = a - np.mean(a)
b = b - np.mean(b)
ds = {'nostim': MorletSpec(a,10e3,0.25,30,0.5), 'stim': MorletSpec(b,10e3,0.25,30,0.5)}

mn,mx = min(np.amin(ds['nostim'].TFR),np.amin(ds['stim'].TFR)),max(np.amax(ds['nostim'].TFR),np.amax(ds['stim'].TFR))

subplot(1,2,1)
imshow(ds['nostim'].TFR[:,10000:],origin='lower',interpolation='None',extent=(1,15,0.25,30),vmin=mn,vmax=mx)
subplot(1,2,2)
imshow(ds['stim'].TFR[:,10000:],origin='lower',interpolation='None',extent=(1,15,0.25,30),vmin=mn,vmax=mx)

davg = {k:np.mean(ds[k].TFR,axis=1) for k in ds.keys()}

plot(ds['nostim'].f,davg['nostim'],'b')
plot(ds['stim'].f,davg['stim'],'r')

xlabel('Frequency (Hz)'); ylabel('Power')
xlim((0.25,30))

savefig('gif/6apr22_dpl_pow_b12.png')

ok, can see a clear 3 Hz peak in stim sim ... 6 Hz harmonic, and higher broadband power too ... no 1/f pattern here

next, try a weaker stimulus ...

simname = 6apr22_StimWeaker_LDB
cfg.netWeight = 10e3

* 7apr22
** check weak stim sim (6apr22_StimWeaker_LDB) outputs, make spectrogram/dipole comparison

python -i simdat.py

simname = '6apr22_StimWeaker_LDB'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)

cant see too much activity in VPL ... 

savefig('gif/7apr22_rast_a0.png') 
cla(); plot(sdat['t'][:],sdat['dipoleSum'][:,0],'g'); 
savefig('gif/7apr22_dpl_a1.png')

lclr = ['b', 'g', 'r']
lsim = ['6apr22_LDB', '6apr22_StimWeaker_LDB', '6apr22_Stim_LDB']
ddat = {}
for simname in lsim:
  simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname)
  ddat[simname] = {'simConfig':simConfig,'sdat':sdat,'dspkID':dspkID,'dspkT':dspkT}
...
for sim in lsim:
  sig = ddat[sim]['sdat']['dipoleSum'][sampr:,0]
  sig = sig - np.mean(sig)
  specg = MorletSpec(sig,sampr,0.25,30,0.5)
  ddat[sim]['specg'] = specg
  avgpow = np.mean(specg.TFR,axis=1)
  ddat[sim]['avgpow'] = avgpow

mn,mx = 1e9,-1e9
for sim in lsim: 
  mn=min(mn, np.amin(ddat[sim]['specg'].TFR))
  mx=max(mx, np.amax(ddat[sim]['specg'].TFR))

mx # 3325186.9565324117
mx = 3325186.9565324117 / 25

for sdx,sim in enumerate(lsim):
  subplot(1,4,sdx+1)
  imshow(ddat[sim]['specg'].TFR[:,10000:],origin='lower',interpolation='None',extent=(1,15,0.25,30),vmin=mn,vmax=mx)
  ylim((0.25,10))

subplot(1,4,4)
for sdx,sim in enumerate(lsim):
  plot(ddat[sim]['specg'].f,ddat[sim]['avgpow'],lclr[sdx])

xlabel('Frequency (Hz)'); ylabel('Power'); xlim((0.25,30))

savefig('gif/7apr22_compare_dpl_specgram_a2.png')

hmm, so the sim with weak stimulation doesnt show a pronounced effect ... since the weak
stimulation is subthreshold and VPL is not too active ... 

could instead treat 6apr22_Stim_LDB as the weak stimulation and add another
sim with snychronized (noise=0) targeted stimulation ... or higher stim than 10e3 
but less than 25e3 ... oh, noticed a mistake in cfg.py -- did not have the 10e3 targeted
stim turned on ... will try that again ... 

** sim with weaker targeted VPL stim (7apr22_Stim10e3_LDB)

./myrun 60

python -i simdat.py

simname = '7apr22_Stim10e3_LDB'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)

savefig('gif/7apr22_rast_b0.png') 
cla(); plot(sdat['t'][:],sdat['dipoleSum'][:,0],'g'); 
savefig('gif/7apr22_dpl_b1.png')

there's a bit of 3 Hz activity from targeted inputs to VPL ... may as well compare to the others ...

lclr = ['b', 'g', 'r']
lsim = ['6apr22_LDB', '7apr22_Stim10e3_LDB', '6apr22_Stim_LDB']
ddat = {}
for simname in lsim:
  simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname)
  ddat[simname] = {'simConfig':simConfig,'sdat':sdat,'dspkID':dspkID,'dspkT':dspkT}
...
savefig('gif/7apr22_compare_dpl_specgram_b2.png')

so the 10e3 weight stim is not strong enough to be noticeably detectable

** sim with 17.5e3 3 Hz targeted weight to VPL (7apr22_Stim17.5e3_LDB)

python -i simdat.py

simname = '7apr22_Stim17.5e3_LDB'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)

ok, can see a clear 3 Hz rhythm in VPL

savefig('gif/7apr22_rast_c0.png') 
cla(); plot(sdat['t'][:],sdat['dipoleSum'][:,0],'g'); 
savefig('gif/7apr22_dpl_c1.png')

lclr = ['b', 'g', 'r']
lsim = ['6apr22_LDB', '7apr22_Stim17.5e3_LDB', '6apr22_Stim_LDB']
ddat = {}
for simname in lsim:
  simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname)
  ddat[simname] = {'simConfig':simConfig,'sdat':sdat,'dspkID':dspkID,'dspkT':dspkT}
...
for sdx,sim in enumerate(lsim):
  subplot(1,4,sdx+1)
  imshow(ddat[sim]['specg'].TFR[:,10000:],origin='lower',interpolation='None',extent=(1,15,0.25,30),vmin=mn,vmax=mx,cmap='jet')
  ylim((0.25,10))
...
xlabel('Frequency (Hz)',fontsize=25); ylabel('Power',fontsize=25); xlim((0.25,25)); ylim((0,155000))

savefig('gif/7apr22_compare_dpl_specgram_c2.png')

for sdx,sim in enumerate(lsim):
  subplot(2,4,sdx+1)
  imshow(ddat[sim]['specg'].TFR[:,10000:],origin='lower',interpolation='None',extent=(1,15,0.25,30),vmin=mn,vmax=mx,cmap='jet')
  ylim((0.25,10)); # xlim((8,12))
  xlabel('Time (s)', fontsize=25); ylabel('Frequency (Hz)',fontsize=25)

for sdx,sim in enumerate(lsim):
  subplot(2,4,sdx+5)
  sig = ddat[sim]['sdat']['dipoleSum'][sampr:,0]
  sig = sig - np.mean(sig)
  plot(np.linspace(1,15,len(sig)),sig,lclr[sdx])
  xlim((11,12)); ylim((-10000,25000))
...
savefig('gif/7apr22_compare_dpl_specgram_c3.png')

also include rasters ...

lt = ['No VPL stimulation', 'Medium strength VPL stimulation (3 Hz)', 'Strong VPL stimulation (3 Hz)']

for sdx,sim in enumerate(lsim):
  subplot(1,3,sdx+1); title(lt[sdx],fontsize=25)
  dspkT,dspkID = ddat[sim]['dspkT'],ddat[sim]['dspkID']
  drawraster(dspkT,dspkID)
  plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
  plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
  xlim((8e3,12e3))
...
savefig('gif/7apr22_compare_raster_c4.png')

there are other ongoing rhythms (e.g. alpha ~10-12 Hz)

save dipoles ... 

lmfn = ['NoVPLStim','MedVPLStim','StrongVPLStim']
for sdx,sim in enumerate(lsim):
  save_dipoles_matlab('../data/'+lmfn[sdx]+'_'+sim+'.mat', ddat[sim]['simConfig'], ddat[sim]['sdat'], dnumc, dstartidx, dendidx)

ok will upload to drive ... 

** next sim -- superthreshold targeted VPL stim with less/0 noise -- (7apr22_Stim17.5e3_Noise0_LDB)
* 8apr22
** 1/f -- where from & why missing here? (sim=8apr22_LDC)

maybe just low-pass filtering of noise
check how spectrogram on LFP signal looks ... 

# setting up LFP recordings
cfg.recordLFP = [[100, y, 100] for y in range(0, 2000, 500)] #+[[100, 2500, 200], [100,2700,200]]
cfg.saveLFPPops =  cfg.S1cells 

./myrun 60

  Done; run time = 2231.41 s; real-time ratio: 0.01.

Gathering data...
[cycle:3983939] *** Process received signal ***
[cycle:3983939] Signal: Segmentation fault (11)
[cycle:3983939] Associated errno: Unknown error 32649 (32649)
[cycle:3983939] Signal code:  (1990185152)
[cycle:3983939] Failing at address: 0xffffffffffffffff
[cycle:3983939] [ 0] /lib/x86_64-linux-gnu/libc.so.6(+0x430c0)[0x7f8b345340c0]

hmm ... try 1 s run first ...

./myrun 60

  Done; run time = 154.25 s; real-time ratio: 0.01.

Gathering data...
terminate called after throwing an instance of 'std::bad_alloc'
  what():  std::bad_alloc
[cycle:3999000] *** Process received signal ***
[cycle:3999000] Signal: Aborted (6)
[cycle:3999000] Signal code:  (-6)
[cycle:3999000] [ 0] /lib/x86_64-linux-gnu/libpthread.so.0(+0x143c0)[0x7f0eee86d3c0]
[cycle:3999000] [ 1] /lib/x86_64-linux-gnu/libc.so.6(gsignal+0xcb)[0x7f0eee6aa03b]
[cycle:3999000] [ 2] /lib/x86_64-linux-gnu/libc.so.6(abort+0x12b)[0x7f0eee689859]

also crashes ...

try with a single LFP electrode ...

./myrun 60

ok, that did not crash ...

python -i simdat.py

simname = '8apr22_LDC'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)

savefig('gif/8apr22_rast_a0.png')

sdat.keys() # dict_keys(['spkt', 'spkid', 'V_soma', 't', 'LFP', 'LFPPops', 'popRates', 'avgRate'])
...
sdat['LFPPops'].keys()
dict_keys(['L1_DAC_bNA', 'L1_DAC_cNA', 'L1_DLAC_cNA', 'L1_HAC_bNA', 'L1_HAC_cIR', 'L1_HAC_cNA', 'L1_NGC-DA_bNA', 'L1_NGC-DA_cAC', 'L1_NGC-DA_cNA', 'L1_NGC-DA_cST', 'L1_NGC-SA_cNA', 'L1_SLAC_bNA', 'L1_SLAC_cAC', 'L1_SLAC_cNA', 'L23_BP_bAC', 'L23_BP_bIR', 'L23_BP_bNA', 'L23_BP_cAC', 'L23_BP_cNA', 'L23_BP_dST', 'L23_BTC_bAC', 'L23_BTC_bIR', 'L23_BTC_bNA', 'L23_BTC_cAC', 'L23_BTC_cNA', 'L23_ChC_cAC', 'L23_ChC_cNA', 'L23_ChC_dNA', 'L23_DBC_bAC', 'L23_DBC_bIR', 'L23_DBC_bNA', 'L23_DBC_cAC', 'L23_LBC_bAC', 'L23_LBC_bNA', 'L23_LBC_cAC', 'L23_LBC_cNA', 'L23_LBC_cST', 'L23_LBC_dNA', 'L23_MC_bAC', 'L23_MC_bNA', 'L23_MC_cAC', 'L23_MC_cNA', 'L23_MC_dNA', 'L23_NBC_bAC', 'L23_NBC_bNA', 'L23_NBC_cAC', 'L23_NBC_cIR', 'L23_NBC_cNA', 'L23_NBC_dNA', 'L23_NGC_bNA', 'L23_NGC_cAC', 'L23_NGC_cNA', 'L23_NGC_cST', 'L23_PC_cAD', 'L23_SBC_bNA', 'L23_SBC_cAC', 'L23_SBC_dNA', 'L4_BP_bAC', 'L4_BP_bIR', 'L4_BP_bNA', 'L4_BP_cAC', 'L4_BP_cNA', 'L4_BP_dST', 'L4_BTC_bAC', 'L4_BTC_bIR', 'L4_BTC_bST', 'L4_BTC_cAC', 'L4_BTC_cNA', 'L4_BTC_dNA', 'L4_ChC_cAC', 'L4_ChC_cNA', 'L4_ChC_dNA', 'L4_DBC_bAC', 'L4_DBC_bIR', 'L4_DBC_bNA', 'L4_DBC_bST', 'L4_DBC_cAC', 'L4_DBC_cIR', 'L4_DBC_cNA', 'L4_LBC_cAC', 'L4_LBC_cNA', 'L4_LBC_cST', 'L4_LBC_dNA', 'L4_LBC_dST', 'L4_MC_bAC', 'L4_MC_bNA', 'L4_MC_cAC', 'L4_MC_cNA', 'L4_MC_dNA', 'L4_NBC_cAC', 'L4_NBC_cIR', 'L4_NBC_cNA', 'L4_NBC_dNA', 'L4_NGC_bNA', 'L4_NGC_cAC', 'L4_NGC_cNA', 'L4_NGC_cST', 'L4_PC_cAD', 'L4_SBC_bNA', 'L4_SBC_cAC', 'L4_SBC_dNA', 'L4_SP_cAD', 'L4_SS_cAD', 'L5_BP_bAC', 'L5_BP_bIR', 'L5_BP_bNA', 'L5_BP_cAC', 'L5_BP_cNA', 'L5_BP_dST', 'L5_BTC_bAC', 'L5_BTC_cAC', 'L5_BTC_cNA', 'L5_ChC_cAC', 'L5_ChC_cNA', 'L5_ChC_dNA', 'L5_DBC_bAC', 'L5_DBC_bIR', 'L5_DBC_bNA', 'L5_DBC_bST', 'L5_DBC_cAC', 'L5_DBC_cIR', 'L5_DBC_cNA', 'L5_LBC_bAC', 'L5_LBC_cAC', 'L5_LBC_cIR', 'L5_LBC_cNA', 'L5_LBC_cST', 'L5_LBC_dNA', 'L5_LBC_dST', 'L5_MC_bAC', 'L5_MC_bIR', 'L5_MC_bST', 'L5_MC_cAC', 'L5_MC_cNA', 'L5_MC_cST', 'L5_MC_dNA', 'L5_NBC_bAC', 'L5_NBC_bIR', 'L5_NBC_bST', 'L5_NBC_cAC', 'L5_NBC_cIR', 'L5_NBC_cNA', 'L5_NBC_cST', 'L5_NBC_dST', 'L5_NGC_bNA', 'L5_NGC_cAC', 'L5_NGC_cNA', 'L5_NGC_cST', 'L5_SBC_bNA', 'L5_SBC_cAC', 'L5_SBC_dNA', 'L5_STPC_cAD', 'L5_TTPC1_cAD', 'L5_TTPC2_cAD', 'L5_UTPC_cAD', 'L6_BPC_cAD', 'L6_BP_bAC', 'L6_BP_bIR', 'L6_BP_bNA', 'L6_BP_cAC', 'L6_BP_cNA', 'L6_BP_dST', 'L6_BTC_bAC', 'L6_BTC_cAC', 'L6_BTC_cNA', 'L6_ChC_cAC', 'L6_ChC_cNA', 'L6_ChC_dNA', 'L6_DBC_bAC', 'L6_DBC_bIR', 'L6_DBC_bNA', 'L6_DBC_bST', 'L6_DBC_cAC', 'L6_DBC_cIR', 'L6_DBC_cNA', 'L6_IPC_cAD', 'L6_LBC_bAC', 'L6_LBC_bIR', 'L6_LBC_bNA', 'L6_LBC_bST', 'L6_LBC_cNA', 'L6_LBC_cST', 'L6_MC_bAC', 'L6_MC_bIR', 'L6_MC_bNA', 'L6_MC_bST', 'L6_MC_cAC', 'L6_MC_cIR', 'L6_MC_cNA', 'L6_NBC_bAC', 'L6_NBC_bIR', 'L6_NBC_bST', 'L6_NBC_cAC', 'L6_NBC_cIR', 'L6_NBC_cNA', 'L6_NBC_cST', 'L6_NBC_dST', 'L6_NGC_bNA', 'L6_NGC_cAC', 'L6_NGC_cNA', 'L6_NGC_cST', 'L6_SBC_bNA', 'L6_SBC_cAC', 'L6_SBC_dNA', 'L6_TPC_L1_cAD', 'L6_TPC_L4_cAD', 'L6_UTPC_cAD'])

plot(sdat['LFPPops']['L4_PC_cAD'])

to get rid of thalamic contributions could either adjust ~/netpyne/netpyne/sim/run.py to allow selection of which
populations contribute ... or  SD suggests recording the thal populations and then just subtract that off the full LFP

or could put a hack ... where only save LFP from cells that have their population LFPs recorded ... 
...
simname = '8apr22_LDC'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); 

plot(sdat['t'],sdat['LFP'])

from morlet import *

sampr = 10000

sig = np.array(sdat['LFP'])
sig = sig - np.mean(sig)
sig = sig[:,0]
specg = MorletSpec(sig,sampr,0.25,30,0.5)
avgpow = np.mean(specg.TFR,axis=1)
...
for sdx,k in enumerate(sdat['LFPPops']):
  subplot(7,2,sdx+1); title(k); plot(sdat['t'],sdat['LFPPops'][k])

sig = np.array(sdat['LFPPops']['L6_TPC_L1_cAD'])
sig = sig - np.mean(sig)
sig = sig[:,0]
specg = MorletSpec(sig,sampr,0.25,30,0.5)
avgpow = np.mean(specg.TFR,axis=1)
figure(); plot(specg.f,avgpow)

for ty,clr in zip(['VPM_sTC', 'VPL_sTC','L4_PC_cAD'],['g','b','r']): plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx[ty])],clr)

* 9apr22
** full scale S1 with LFP crashes
...
* 11apr22
** debug reduced model LFP recording (11apr22_LDA)

./myrun 60

sim.simData.keys() # dict_keys(['spkt', 'spkid', 'V_soma', 't', 'LFP', 'LFPPops'])

sim.simData['LFPPops'].keys()
plot(sim.simData['LFPPops']['L4_PC_cAD'])
np.amin(sim.simData['LFPPops']['L4_PC_cAD']),np.amax(sim.simData['LFPPops']['L4_PC_cAD']) # (0.0, 0.0)
len(sim.simData['LFPPops']['L4_PC_cAD']) # 4000
np.amin(sim.simData['LFP']),np.amax(sim.simData['LFP']) # (-0.055766799449415656, 0.03242565360461221)

python -i simdat.py

simname = '11apr22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); 
...
#for sdx,k in enumerate(sdat['LFPPops']):
#  subplot(7,2,sdx+1); title(k); plot(sdat['LFPPops'][k])

dnumc['L4_PC_cAD'] # 19

pull/restore development branch of netpyne?

cd ~/netpyne
git pull origin development
pip install -e .

gather.py, run.py were not updated ... 

but utils.py, wrappers.py were updated ... 

for ty,clr in zip(['VPM_sTC', 'VPL_sTC','L4_PC_cAD'],['g','b','r']): plot(sdat['t'],sdat['V_soma']['cell_'+str(dstartidx[ty])],clr)

hmm, psd looks more 1/f like ... 

tried running 15 s sim but it crashed in gather
...
try w/o LFPPops and w/o LFPCells ...

ok, that ran to completion ...

* 13apr22
** check last output (12apr22_LDA)

python -i simdat.py

simname = '12apr22_LDA'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); 

plot(sdat['LFP'])
savefig('gif/13apr22_lfp_a0.png')

drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/13apr22_rast_a1.png')

from morlet import *

sampr = 10000

sig = np.array(sdat['LFP'][1000:])
sig = sig - np.mean(sig)
sig = sig[:,0]
specg = MorletSpec(sig,sampr,0.25,30,0.5)
avgpow = np.mean(specg.TFR,axis=1)
plot(specg.f,avgpow)
xlabel('Freq (Hz)',fontsize=25); ylabel('Average wavelet power',fontsize=25)

savefig('gif/13apr22_wvlt_avg_a2.png')

from psd import psd
figure(); fpcntile, mpf, fmax, Ptotal, f, P = psd(sig, sampr)
xlim((0,3000))

savefig('gif/13apr22_welch_psd_a3.png')

xlim((0,30))

savefig('gif/13apr22_welch_psd_a4.png')

specg = MorletSpec(sig,sampr,0.25,3000,10)
avgpow = np.mean(specg.TFR,axis=1)
plot(specg.f,avgpow)

savefig('gif/13apr22_wvlt_avg_a4.png')

np.savetxt('testLFP.txt', sig)

so at wide freq range looks more similar to 1/f

** another variation (12apr22_LDB)

use the lower sizeX, sizeZ (420/12.), and record LFP from middle at x=z=17.5

python -i simdat.py

simname = '12apr22_LDB'; simConfig, sdat, dstartidx, dendidx, dnumc, dspkID, dspkT = loaddat(simname); 

plot(sdat['LFP'])
savefig('gif/13apr22_lfp_b0.png')

drawraster(dspkT,dspkID)
plot([0,totalDur],[dstartidx['VPL_sTC'],dstartidx['VPL_sTC']],'k--',linewidth=2)
plot([0,totalDur],[dendidx['VPL_sTC'],dendidx['VPL_sTC']],'k--',linewidth=2)
savefig('gif/13apr22_rast_b1.png')
...
savefig('gif/13apr22_wvlt_avg_b2.png')

from psd import psd
fpcntile, mpf, fmax, Ptotal, f, P = psd(sig, sampr)
xlim((0,3000))

savefig('gif/13apr22_welch_psd_b3.png')

specg = MorletSpec(sig,sampr,0.25,3000,1)
avgpow = np.mean(specg.TFR,axis=1)
plot(specg.f,avgpow)
xlabel('Freq (Hz)',fontsize=25); ylabel('Average wavelet power',fontsize=25)
