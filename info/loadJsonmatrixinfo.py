    ...
    for post in popParamLabels:
        ...
        if proj in data['BBP_S1']['connProb']:
            ...
            if A_literature > 0.000001:
                ...
                matrixprob[i-1,j-1] = 100*A_literature
            else:
                R0 = 420 # small pops have prob(< 100um) smaller the 1 percent, but there are connections to  > 100um
                # use all connections to aproxx this prob.
                numberofconnections = mtypeNumber[pre]*data['BBP_S1']['connProb'][proj]['number_of_divergent_neuron_mean']
                if pre == post:
                    totalpossible = mtypeNumber[pre]*(mtypeNumber[post]-1)
                else:
                    totalpossible = mtypeNumber[pre]*mtypeNumber[post]
                
                A_literature =  numberofconnections/totalpossible
                # print(proj,A_literature)

                A0 = A_literature # uniform prob
                if A0 > 1.0: A0 = 1.0  # make max prob = 1.0
                data['BBP_S1']['connProb'][proj]['A0'] = A0
                data['BBP_S1']['connProb'][proj]['sigma'] = 0.0 # uniform prob
                matrixprob[i-1,j-1] = 100*A_literature


# np.savetxt('matrix_prob_pop.txt',matrixprob, fmt='%f', delimiter=" ")
