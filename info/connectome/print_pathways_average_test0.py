    ...
    for n in range(1,56):
        for m in range(1,56):
            ...
            if mm == 7:
                ...
                x0 = d2D[starts:ends]
                y0 = prob2D[starts:ends]

                pars0, cov = curve_fit(f=exponential, xdata=x, ydata=y, p0=[0, 0], bounds=(-np.inf, np.inf))
                ...
                if x[0] == 12.5:
                    xx0 = np.linspace(0, endsexp, 50)
                    yy0 = exponential(xx0, *pars0)
                    x0i = -1
                    y0i = 0
                else:
                    xx0 = np.linspace(x[0], endsexp, 50)
                    yy0 = exponential(xx0, *pars0)
                             
                    if x[0] == 25:
                        x0i = [0, 25]
                        y0i = [prob2D[0], prob2D[0]]
                    elif x[0] == 50:
                        x0i = [0, 50]
                        y0i = [prob2D[1], prob2D[1]]
                    else:
                        y1 = prob2D[1]
                        x1 = 25
                        y2 = y[0]
                        ...
                        a = (y2 - y1)/(x2 - x1)
                        b = y2 - x2*a    
                        x0i = np.linspace(0, x[0], 11)
                        y0i = linear(x0i,a, b)

                cov0 = 1000000*(cov[0][0]+cov[0][1]+cov[1][0]+cov[1][1])
                ...
                print('%s:%s %d %d %.5f %.3f %.1f %.3f %.1f %.0f %.2f' % (mtype_map[n],mtype_map[m],n,m,(cov0/cov1),pars0[0],(1.0/pars0[1]),pars[0],(1.0/pars[1]),(nn/mm),(pmat/mm)))
            ...
            if mm == 7:                
                ...
                Netinfo[proj]['A00'] = '%.5f' % pars0[0]
                Netinfo[proj]['shape0'] = '%.2f' % (1.0/pars0[1])
                Netinfo[proj]['d_init0'] = '%.1f' % x0[0] 
                ...
                Netinfo[proj]['cov0_cov'] = '%.1f' % (cov0/cov1)

                # plot probs
                if plotdata:
                # if prob2D[1] == 0 or prob2D[2] == 0 or prob2D[3] == 0:
                    fontsiz=18
                    figSize = (12,8)
                    fig = plt.figure(figsize=figSize)  # Open a new figure
                    fig.suptitle('%s -> A0=%.3f(%.3f), l=%.1f(%.1f), %0.1f(%0.1f)<d<%d, NC=%.1f, cov=%.1f(%.1f)e-6' % (proj,pars0[0],pars[0],1.0/pars0[1],1.0/pars[1],x0[0],x[0],x0[-1],nn/mm,cov0,cov1), fontsize=13, fontweight='bold')
                    # fig.suptitle('%s -> (cov0=%.2f, cov=%.2f, %0.1f<d<%d, NC=%.1f)' % (proj,cov0,cov1,x[0],x[-1],nn/mm), fontsize=18, fontweight='bold')
                    
                    plt.subplot(1, 1, 1)
                    plt.ylabel('Prob2D', fontsize=fontsiz)
                    plt.plot(d2Dfulldata, prob2Dfulldata, 'k.', label='7mcs')
                    plt.plot(d2D, prob2D, 'k-o', label='mean')
                    plt.plot(x0, y0, 'ro', label='used0') 
                    plt.plot(xx0, yy0, 'r-', label='fit0') 
                    plt.plot(x0i, y0i, 'r-') 
